...
async function createServer(isProd = process.env.NODE_ENV === "production") {
	const app = express();
	// Create Vite server in middleware mode and configure the app type as 'custom', disabling Vite's own HTML serving logic so parent server can take control
	...
	// use vite's connect instance as middleware if you use your own express router (express.Router()), you should use router.use
	...
	app.use("*", async (req: Request, res: Response, next: NextFunction) => {
		...
		try {
			...
			// 3. Load the server entry. vite.ssrLoadModule automatically transforms your ESM source code to be usable in Node.js! There is no bundling required, and provides efficient invalidation similar to HMR.
			...
			// 4. render the app HTML. This assumes entry-server.js's exported `render` function calls appropriate framework SSR APIs,
